...
var deleteRoute = function (app, protocol, url) {
    var targetUrl = url;
    if (targetUrl.indexOf("/") != 0 ) {
        targetUrl = "/" + targetUrl;
    }
    for (var i = app.routes[protocol].length - 1; i >= 0; i--) {
        if (app.routes[protocol][i].path === "/" + targetUrl) {
            app.routes[protocol].splice(i, 1);
        }
    }    
}
...
var onAppsFolderChange = function(err, data) {
    if (data.action == "add") {
        ...
        // remove from server's route and vhost?/
        // deleteRoute(settings.server, protoco)
    }
}
/*
